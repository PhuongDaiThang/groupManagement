...
Bảng `users` lưu trữ thông tin về tất cả các người dùng trong hệ thống. Đây là bảng quan trọng nhất vì nó xác định danh tính của người dùng và liên kết với nhiều bảng khác.

- **id** (Primary Key): Mã định danh duy nhất cho mỗi người dùng.
- **username** (Unique): Tên đăng nhập của người dùng, không trùng lặp.
- **password**: Mật khẩu của người dùng, được mã hóa.
- **email**: Địa chỉ email của người dùng, dùng để liên lạc và xác thực.
- **full_name**: Tên đầy đủ của người dùng.
- **bio**: Thông tin ngắn về bản thân người dùng.
- **profile_pic**: Đường dẫn đến ảnh đại diện của người dùng.
- **created_at**: Thời gian tạo tài khoản.

## Bảng `groups`

Bảng `groups` lưu trữ thông tin về các nhóm trong hệ thống. Một nhóm có thể được tạo ra bởi bất kỳ người dùng nào và có thể chứa nhiều thành viên.
...
- **created_at**: Thời gian tạo nhóm.

## Bảng `group_members`

Bảng `group_members` lưu trữ thông tin về các thành viên trong mỗi nhóm. Bảng này xác định ai là thành viên của nhóm nào và vai trò của họ.
...
- **is_admin** (Boolean): Xác định xem người dùng có phải là quản trị viên của nhóm không.
- **joined_at**: Thời gian người dùng tham gia nhóm.

## Bảng `group_invitations`

Bảng `group_invitations` lưu trữ các lời mời tham gia nhóm. Khi một người dùng muốn mời người khác tham gia nhóm, lời mời này sẽ được lưu trong bảng này.
...
- **status** (Pending/Accepted/Rejected): Trạng thái của lời mời.
- **invited_at**: Thời gian gửi lời mời.
- **responded_at**: Thời gian phản hồi lời mời.

## Bảng `group_roles`

Bảng `group_roles` lưu trữ thông tin về vai trò của người dùng trong các nhóm. Nó xác định người dùng có vai trò gì trong một nhóm cụ thể.
...
Bảng `comments` lưu trữ các bình luận của người dùng trên các bài viết. Mỗi bình luận được liên kết với một bài viết cụ thể, có thể là của nhóm hoặc cá nhân.
...
Bảng `files` lưu trữ các tệp đính kèm được tải lên cùng với bài viết. Tệp này có thể là hình ảnh, tài liệu, hoặc các loại tệp khác hỗ trợ bài viết.
